...
Build a production-ready ServiceNow scoped application that leverages ai classification to automatically tag and categorize configuration items (cis) in the servicenow cmdb, improving data quality and relationship mapping accuracy.. The application must be installable on any Australia-era ServiceNow instance and must pass automated functional tests with zero external runtime dependencies.
